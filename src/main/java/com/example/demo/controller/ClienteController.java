...
public class ClienteController {
    ...
    private boolean isValid(Map<String, String> credenciales, String... requiredFields) {
        if (credenciales == null) return false;
        for (String field : requiredFields) {
            if (!credenciales.containsKey(field) || credenciales.get(field).trim().isEmpty()) {
                return false;
            }
        }
        return true;
...
public ResponseEntity<?> login(@RequestBody Map<String, String> credenciales) {
    ...
    if (!isValid(credenciales, "email", "password")) {
            return ResponseEntity.badRequest().body("Debe proporcionar email y password.");
        }
        String email = credenciales.get("email");
        String password = credenciales.get("password");

    boolean isAuthenticated = clienteService.authenticate(email, password);
    ...
    public ResponseEntity<?> register(@RequestBody Map<String, String> credenciales) {
        ...
        Cliente nuevoUsuario = clienteService.register(email, password,nombre);
